...
def train_challenge_model(data_folder, model_folder, verbose, iters=1000, depth=6, importance=1.0, learning_rate=0.05) -> None:
    ...
    for i in range(2):
        ...
        model = CatBoostClassifier(
            iterations=iters,                       # Number of iterations
            depth=depth,                            # Depth of each tree to prevent overfitting
            learning_rate=learning_rate,            # Step size of udpates
        ...
        if i == 0:
            ...
            important_features = importances[importances['Importances'] >= importance]['Feature Id'].tolist()
